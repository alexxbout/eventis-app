<template>
    <div v-if="props.style == EUserCardStyle.DEFAULT" ref="container" class="h-[137px] w-full max-w-xs rounded-[13px] bg-[#FAFAFA] p-[13px] flex flex-col gap-y-[10px]">
        <!-- Picture + button -->
    ...
    <div v-else class="w-full h-max flex items-center justify-between gap-x-5">
        <div class="flex items-center justify-center w-max gap-x-5">
            <div v-if="props.data.pic" class="bg-no-repeat rounded-full w-[60px] h-[60px] bg-center bg-cover" :style="{ backgroundImage: 'url(http://localhost:8080/image/users/' + props.data.pic + ')' }"></div>
            <span>{{ props.data.firstname + ' ' + props.data.lastname }}</span>
        </div>

        <Button class="w-max" :data="{ size: 'BASE', color: 'BLUE', type: 'PRIMARY', text: 'Profil' }" />
    </div>
</template>
  
<script setup lang="ts">
import Button from "../Button.vue";
import { Prop, PropType, onMounted, ref } from "vue";
...
const props = defineProps({
    ...
    style: {
        type: Number as PropType<EUserCardStyle>,
        // default: EUserCardStyle.DEFAULT
    }
});
